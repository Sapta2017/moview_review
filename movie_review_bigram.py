...
#Create Word Features from Text
...
def create_word_features(words):
 ...
 def create_bigram_features(words):
...
for fileid in movie_reviews.fileids('neg'):
    ...
    useful_words = [word for word in words if word not in stopwords.words("english") if word not in string.punctuation ]
    finder = BigramCollocationFinder.from_words(useful_words)
    scored = finder.score_ngrams(bigram_measures.raw_freq)
    bigram=sorted(bigram for bigram, score in scored)
    neg_reviews_bi.append((create_bigram_features(bigram), "negative"))
print(len(neg_reviews_bi))
...
for fileid in movie_reviews.fileids('pos'):
    ...
    scored = finder.score_ngrams(bigram_measures.raw_freq)
    bigram=sorted(bigram for bigram, score in scored)
    pos_reviews_bi.append((create_bigram_features(bigram), "positive"))
print(len(pos_reviews_bi))

train_set_bi = neg_reviews_bi[:750] + pos_reviews_bi[:750]
test_set_bi =  neg_reviews_bi[750:] + pos_reviews_bi[750:]
print(len(train_set_bi),  len(test_set_bi))

classifier = NaiveBayesClassifier.train(train_set_bi)

accuracy = nltk.classify.util.accuracy(classifier, test_set_bi)
print(accuracy * 100)
...
useful_words = [word for word in words if word not in stopwords.words("english") if word not in string.punctuation ]
words = create_bigram_features(useful_words)
